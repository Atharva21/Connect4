...
class Grid:
    ...
    def placeMark(self):
        ...
        if self.isP1Playing:
            ...
            # tmp = self.canvas.create_oval((self.xIndex*100)+20,
            # (self.yIndex*100) +20, (self.xIndex*100)+80, (self.yIndex*100)+80, fill = 'red')
            # self.markIds[self.yIndex][self.xIndex] = tmp

            self.gameState[self.yIndex][self.xIndex] = 1
            tmp = self.canvas.create_oval((self.xIndex * 100) + 20, (self.yIndex * 100) - 680, (self.xIndex * 100) + 80,
                                          (self.yIndex * 100) - 620, fill='red')

            if self.checkWin() or self.roundCount == 36:
                self.click_able = False
            Thread(target=self.dropInAnimation, args=(tmp,)).start()
            ...
            # tmp = self.canvas.create_oval((self.xIndex*100)+20,
            # (self.yIndex*100) +20, (self.xIndex*100)+80, (self.yIndex*100)+80, fill = 'blue')
            # self.markIds[self.yIndex][self.xIndex] = tmp

            self.gameState[self.yIndex][self.xIndex] = 2
            tmp = self.canvas.create_oval((self.xIndex * 100) + 20, (self.yIndex * 100) - 680, (self.xIndex * 100) + 80,
                                          (self.yIndex * 100) - 620, fill='blue')
    ...
    def checkWin(self):
        ...
        for i in range(3):
            for j in range(6):
                # vertical
                if (self.gameState[i][j] != 0 and (
                        self.gameState[i][j] == self.gameState[i + 1][j] == self.gameState[i + 2][j] ==
                    ...
                    break
                # horizontal
                if (self.gameState[j][i] != 0 and (
                        self.gameState[j][i] == self.gameState[j][i + 1] == self.gameState[j][i + 2] ==
                        self.gameState[j][i + 3])):
                    win = 2
                    x = i
                    y = j
                    break
            ...
            for i in range(3):
                for j in range(3):
                    if self.gameState[i][j] != 0:
                        if (self.gameState[i][j] == self.gameState[i + 1][j + 1] == self.gameState[i + 2][j + 2] ==
                    ...
                    if self.gameState[i][k] != 0:
                        if (self.gameState[i][k] == self.gameState[i + 1][k - 1] == self.gameState[i + 2][k - 2] ==
                                self.gameState[i + 3][k - 3]):
                            win = 4
                            x = i
                            y = k
                            break
        ...
        if win == 0:
            ...
            if win == 1:
                self.winningMarks = [self.markIds[x][y], self.markIds[x + 1][y], self.markIds[x + 2][y],
                                     self.markIds[x + 3][y]]
            elif win == 2:
                self.winningMarks = [self.markIds[y][x], self.markIds[y][x + 1], self.markIds[y][x + 2],
                                     self.markIds[y][x + 3]]
            elif win == 3:
                self.winningMarks = [self.markIds[x][y], self.markIds[x + 1][y + 1], self.markIds[x + 2][y + 2],
                                     self.markIds[x + 3][y + 3]]
            elif win == 4:
                self.winningMarks = [self.markIds[x][y], self.markIds[x + 1][y - 1], self.markIds[x + 2][y - 2],
                                     self.markIds[x + 3][y - 3]]
    ...
    def p1Wins(self):
        ...
        txt = self.canvas.create_text(self.WIDTH / 2, self.HEIGHT / 2, text="Red Wins!!", font="Consolas 34 bold",
                                      fill="red")
        self.canvas.update()
        time.sleep(1.5)
    ...
    def p2Wins(self):
        self.blinkingAnimation()
        txt = self.canvas.create_text(self.WIDTH / 2, self.HEIGHT / 2, text="Blue Wins!!", font="Consolas 34 bold",
                                      fill="blue")
        self.canvas.update()
        time.sleep(1.5)
        self.canvas.delete(txt)
    ...
    def draw(self):
        txt = self.canvas.create_text(self.WIDTH / 2, self.HEIGHT / 2, text="Draw!", font="Consolas 34 bold",
                                      fill="white")
        self.canvas.update()
        time.sleep(1.5)
        self.canvas.delete(txt)
    ...
    def dropInAnimation(self, id):
        ...
        click_able = self.click_able
        start_y = self.canvas.coords(id)[1]
        start_time = time.monotonic()

        # change move y to change speed don't change sleep time :)
        ...
        while self.canvas.coords(id)[1] < start_y + 700:
            try:
                ...
                self.canvas.update()
                self.canvas.move(id, 0, 8)
            except:
                pass

            if time.monotonic() - start_time == 0.0:
                time.sleep(0.015)
            start_time = time.monotonic()
